 ...
 */
static uint32_t		mts_devseed(mt_state* state, const char* seed_dev);
...
int mts_savestate(
	...
	char temp[100];
	sprintf(temp, "%s%s ", "%", PRIu32);
    for (i = MT_STATE_SIZE;  --i >= 0;  )
	{
	if (fprintf(statefile, temp, state->statevec[i]) < 0)
	    return 0;
	}
...
int mts_loadstate(
	...
	char temp[100];
	sprintf(temp, "%s%s", "%", SCNu32);
    for (i = MT_STATE_SIZE;  --i >= 0;  )
	{	
	if (fscanf(statefile, temp, &state->statevec[i]) != 1)
	    return 0;
	}
